...
# One special method is also implemented to allow for indexing of children in the tree:
#      __getitem__ : allows to get the value from the Node class 
...
def dfs(current, score):
    ...
    if type(sol_pos_val) is int: # if the position is an integer (i.e. target matrix), implement score variable
        if sol_pos_val == 0:
            score-=1
    else:
